...
public class FriendManager {
    ...
    public void sendFriendRequest(Player player, Player friend) {
        if (player.getUniqueId().equals(friend.getUniqueId())) {
            player.sendMessage(org.bukkit.ChatColor.RED + "You cannot add yourself as a friend.");
            return;
        }
        if (friends.getOrDefault(player.getUniqueId(), Collections.emptyList()).contains(friend.getUniqueId())) {
            player.sendMessage(org.bukkit.ChatColor.RED + "You are already friends with " + friend.getName() + ".");
            return;
        }
        if (friendRequests.containsKey(friend.getUniqueId()) && friendRequests.get(friend.getUniqueId()).equals(player.getUniqueId())) {
            ...
            player.sendMessage(org.bukkit.ChatColor.GREEN + "You are now friends with " + friend.getName() + ".");
            friend.sendMessage(org.bukkit.ChatColor.GREEN + "You are now friends with " + player.getName() + ".");
            friendRequests.remove(friend.getUniqueId());
        } else {
            friendRequests.put(player.getUniqueId(), friend.getUniqueId());
            player.sendMessage(org.bukkit.ChatColor.GREEN + "Friend request sent to " + friend.getName() + ".");
            ...
            friend.spigot().sendMessage(new TextComponent(ChatColor.AQUA + player.getName() + " has sent you a friend request! "), acceptButton, new TextComponent(" "), denyButton);
        }
    }
    ...
    public void acceptFriendRequest(Player player, Player friend) {
        ...
        if (friendRequests.containsKey(friendId) && friendRequests.get(friendId).equals(player.getUniqueId())) {
            ...
            player.sendMessage(org.bukkit.ChatColor.RED + "No friend request from " + friend.getName() + ".");
        }
    }
    ...
    public void denyFriendRequest(Player player, Player friend) {
        UUID friendId = friend.getUniqueId();
        if (friendRequests.containsKey(friendId) && friendRequests.get(friendId).equals(player.getUniqueId())) {
            player.sendMessage(org.bukkit.ChatColor.RED + "Friend request from " + friend.getName() + " denied.");
            friend.sendMessage(org.bukkit.ChatColor.RED + player.getName() + " has denied your friend request.");
            friendRequests.remove(friendId);
        } else {
            player.sendMessage(org.bukkit.ChatColor.RED + "No friend request from " + friend.getName() + ".");
        }
    ...
    public void removeFriend(Player player, Player friend) {
        ...
        if (playerFriends != null && playerFriends.contains(friendId)) {
            ...
            player.sendMessage(org.bukkit.ChatColor.GREEN + "You have removed " + friend.getName() + " from your friends.");
            if (friendFriends != null && friendFriends.contains(playerId)) {
                friendFriends.remove(playerId);
                friend.sendMessage(org.bukkit.ChatColor.RED + player.getName() + " has removed you from their friends.");
            }
            saveFriends();
        } else {
            player.sendMessage(org.bukkit.ChatColor.RED + friend.getName() + " is not in your friends list.");
        }
    }
    ...
    public void removeIgnoredPlayerFromFriends(Player player, Player ignored) {
        UUID playerId = player.getUniqueId();
        UUID ignoredId = ignored.getUniqueId();
        List<UUID> playerFriends = friends.get(playerId);
        List<UUID> ignoredFriends = friends.get(ignoredId);
        if (playerFriends != null && playerFriends.contains(ignoredId)) {
            playerFriends.remove(ignoredId);
            if (ignoredFriends != null && ignoredFriends.contains(playerId)) {
                ignoredFriends.remove(playerId);
            }
            saveFriends();
        }
    }
